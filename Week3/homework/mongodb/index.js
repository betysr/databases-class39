...
async function findEpisodesExercises(client) {
  ...
  const res1 = await cursor1.toArray();
  const result1 = res1.map((each) => each.title).toString();
  console.log(
    `The episodes that Bob Ross painted a CLIFF are ${result1}`
  );
  ...
  const res2 = await cursor2.toArray();
  const result2 = res2
    .map((eps) => eps.title)
    .toString();

  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${result2}`
  );
}
...
async function deleteEpisodeExercise(client) {
  ...
  const check = await client
    ...
    if(!check && res.deletedCount>0) {
      console.log(` This episode was deleted and confirmed.`)}
